...
CREATE INDEX IF NOT EXISTS idx_orders_creator_code ON orders(creator_code);
...
CREATE INDEX IF NOT EXISTS idx_creator_trans_creator_id ON creator_transactions(creator_id);
CREATE INDEX IF NOT EXISTS idx_creator_trans_order_id ON creator_transactions(order_id);
CREATE INDEX IF NOT EXISTS idx_creator_trans_status ON creator_transactions(status);
CREATE INDEX IF NOT EXISTS idx_creator_trans_created_at ON creator_transactions(created_at);
...
CREATE INDEX IF NOT EXISTS idx_referral_creator_id ON referral_tracking(creator_id);
...
DROP POLICY IF EXISTS "ebooks_select_public" ON ebooks;
...
DROP POLICY IF EXISTS "creator_transactions_select_auth" ON creator_transactions;
CREATE POLICY "creator_transactions_select_auth" ON creator_transactions
  FOR SELECT USING (auth.role() = 'authenticated');

DROP POLICY IF EXISTS "creator_transactions_insert_auth" ON creator_transactions;
CREATE POLICY "creator_transactions_insert_auth" ON creator_transactions
  FOR INSERT WITH CHECK (auth.role() = 'authenticated');

DROP POLICY IF EXISTS "creator_transactions_update_auth" ON creator_transactions;
CREATE POLICY "creator_transactions_update_auth" ON creator_transactions
  FOR UPDATE USING (auth.role() = 'authenticated')
  WITH CHECK (auth.role() = 'authenticated');

-- Referral tracking policies
DROP POLICY IF EXISTS "referral_tracking_select_auth" ON referral_tracking;
CREATE POLICY "referral_tracking_select_auth" ON referral_tracking
  FOR SELECT USING (auth.role() = 'authenticated');

DROP POLICY IF EXISTS "referral_tracking_insert_auth" ON referral_tracking;
CREATE POLICY "referral_tracking_insert_auth" ON referral_tracking
  FOR INSERT WITH CHECK (auth.role() = 'authenticated');

-- ============================================================================
-- SAMPLE DATA: Skipped — column names differ from live schema.
-- ============================================================================
